...
local Module = D:NewModule('Emojis', 'AceEvent-3.0', 'AceHook-3.0')
...
local getEmojiMedia = 'Interface\\AddOns\\DuffedUI\\media\\emojis\\'
...
function Module:SetupEmojis(_, msg)
	for word in string_gmatch(msg, '%s-%S+%s*') do
		...
		if emoji and string_match(msg, '[%s%p]-'..pattern..'[%s%p]*') then
			...
			local base64 = D['Base64']:Encode(word)
...
function Module:OnInitialize() self.CreateEmojis() end